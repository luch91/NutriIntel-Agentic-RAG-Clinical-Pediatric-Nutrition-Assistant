...
def _build_and_ingest():
    ...
    if not pdf_paths:
        logger.warning("startup_ingestion: no PDFs found in %s — retrieval will be empty", _DATA_DIR)
    else:
        logger.info("startup_ingestion: ingesting %d PDFs from %s", len(pdf_paths), _DATA_DIR)

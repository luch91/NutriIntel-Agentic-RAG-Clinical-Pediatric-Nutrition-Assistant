...
OXFORD_TOC = {
    22: {"title": "Type 1 Diabetes", "pages": (441, 464), "authors": []},
    26: {"title": "GI Disorders", "pages": (561, 610), "authors": []},
    29: {"title": "Chronic Kidney Disease", "pages": (631, 657), "authors": []},
    30: {"title": "Cystic Fibrosis", "pages": (657, 662), "authors": []},
    33: {"title": "Epilepsy", "pages": (698, 700), "authors": []},
    36: {"title": "Inherited Metabolic Disorders (PKU/MSUD/Galactosemia)", "pages": (719, 728), "authors": []},
    37: {"title": "Food Allergy", "pages": (729, 736), "authors": []},
}

# Clinical Nutrition: The Nutrition Society Textbook Series (2013)
_CLINICAL_NUTRITION_STARTS = [
    ...
    (19, "Nutritional Aspects of Disease Affecting the Skeleton", 327),
...
# Vitamins and Mineral Requirements in Human Nutrition
_VIT_MIN_STARTS = [
    (1, "Concepts, definitions and approaches used to define nutritional needs and recommendations", 1),
    (2, "Vitamin A", 17),
    (3, "Vitamin D", 45),
    (4, "Calcium", 59),
    (5, "Vitamin E", 94),
    ...
    (9, "Thiamine, riboflavin, niacin, vitamin B6, pantothenic acid, and biotin", 164),
    (10, "Selenium", 194),
    (11, "Magnesium", 217),
    (12, "Zinc", 230),
    (13, "Iron", 246),
    ...
    (18, "Annex 1: Recommended nutrient intakes — minerals", 338),
]


def _build_toc_from_starts(starts):
    toc = {}
    for idx, (ch_num, title, start) in enumerate(starts):
        next_start = starts[idx + 1][2] if idx + 1 < len(starts) else None
        end = (next_start - 1) if next_start else None
        toc[ch_num] = {"title": title, "pages": (start, end), "authors": []}
    return toc


CLINICAL_NUTRITION_TOC = _build_toc_from_starts(_CLINICAL_NUTRITION_STARTS)
VITAMIN_MINERAL_TOC = _build_toc_from_starts(_VIT_MIN_STARTS)
...
# ==============================================================================
# West Africa Food Composition Table (2019) — CANONICAL FCT
# ==============================================================================
# Source: Food Composition Table for West Africa
# (Ghana, Senegal, Mali, Burkina Faso, Nigeria, Cote d'Ivoire) 2019
# Pages 60–422 contain food composition data tables.
# Used as the canonical FCT for African pediatric meal planning.

WEST_AFRICA_FCT_TOC = {
    1:  {"title": "Cereals and their products",                    "pages": (60, 123)},
    2:  {"title": "Starchy roots, tubers and their products",      "pages": (124, 151)},
    3:  {"title": "Legumes and their products",                     "pages": (152, 191)},
    4:  {"title": "Vegetables and their products",                  "pages": (192, 235)},
    5:  {"title": "Fruits and their products",                      "pages": (236, 263)},
    6:  {"title": "Meat, poultry and their products",               "pages": (264, 319)},
    7:  {"title": "Eggs and their products",                        "pages": (320, 323)},
    8:  {"title": "Fish and its products",                          "pages": (324, 351)},
    9:  {"title": "Milk and its products",                          "pages": (352, 367)},
    10: {"title": "Fats and oils",                                  "pages": (368, 391)},
    11: {"title": "Beverages",                                      "pages": (392, 403)},
    12: {"title": "Miscellaneous",                                  "pages": (404, 414)},
    13: {"title": "Soups and sauces",                               "pages": (414, 423)},
}
...
def extract_chapters_from_pdf(file_path: str, doc_type: str) -> List[Document]:
    ...
    toc_map = {
        ...
        "oxford_handbook": ("OxfordHandbook2012", "Oxford Handbook of Nutrition and Dietetics (2012, Oxford University Press)", OXFORD_TOC),
        "clinical_nutrition_2013": ("ClinicalNutrition2013", "Clinical Nutrition: The Nutrition Society Textbook Series (2013, Wiley-Blackwell)", CLINICAL_NUTRITION_TOC),
        "vitamin_mineral_requirements": ("VitMinRequirements", "Vitamins and Mineral Requirements in Human Nutrition", VITAMIN_MINERAL_TOC),
    ...
    for chapter_key, chapter_info in toc.items():
        ...
        if page_end > len(all_pages):
            page_end = len(all_pages)

        # Validate page range
        if page_start < 1 or page_start > len(all_pages):
            logger.warning(f"Invalid page start for chapter {chapter_num}: {page_start} vs {len(all_pages)} pages")
            continue
        ...
        logger.info(f"OK: Extracted {chunk_type.capitalize()} {chapter_num}: {chapter_info['title']} ({page_end - page_start + 1} pages, {len(chapter_text)} chars)")
...
def get_toc_for_document(doc_type: str) -> Dict[Any, Dict[str, Any]]:
    ...
    Args:
        doc_type: One of ["dri", "shaw_2020", "oxford_handbook", "clinical_nutrition_2013", "vitamin_mineral_requirements", "preterm_2013", "drug_nutrient", "biochemistry"]
    ...
    toc_map = {
        "dri": DRI_TOC,
        "shaw_2020": SHAW_TOC,
        "oxford_handbook": OXFORD_TOC,
        "clinical_nutrition_2013": CLINICAL_NUTRITION_TOC,
        "vitamin_mineral_requirements": VITAMIN_MINERAL_TOC,
